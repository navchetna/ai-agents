...
GROQ_SERVICE_URL = f"http://{SERVER_HOST_URL}/v1/chat/completions"
...
class RAGConversation:    
    ...
    async def _call_groq_service(self, messages: List[Dict], max_tokens: int, temperature: float) -> str:
        async with httpx.AsyncClient() as client:
            try:
                ...
                print(f"Error calling Groq service: {str(e)}")
                raise
    ...
    async def generate_response(self, 
        ...
        try:
            ...
            response_content = await self._call_groq_service(
                messages=messages,
                max_tokens=max_tokens,
                temperature=temperature
            )
