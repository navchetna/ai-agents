...
import { ChromeIcon as Google, Mail, ChevronLeft, ChevronRight, Star, Search } from "lucide-react"
import {
  format,
  addMonths,
  subMonths,
...
export function IntegratedCalendar() {
  ...
  const handleFilterChange = (key: keyof FilterState, value: string) => {
    setFilters((prev) => {
      ...
      if (Object.values(newFilters).every((v) => !v || v === "All Groups" || v === "All")) {
        resetEvents()
      }
      return newFilters
    })
  ...
  const handleFindEvents = () => {
    const filteredEvents = events.filter((event) => {
      const matchesDate = filters.date ? isSameDay(event.start, new Date(filters.date)) : true
      const matchesKeyword = filters.keyword ? event.title.toLowerCase().includes(filters.keyword.toLowerCase()) : true
      const matchesGroup = filters.group !== "All Groups" ? event.type === filters.group.toLowerCase() : true
      const matchesType = filters.type !== "All" ? event.type === filters.type.toLowerCase() : true
                ...
                <div className="flex-1 overflow-hidden">
                  ...
                  <div className="flex flex-col gap-px mt-1">
                    {dayEvents.slice(0, 3).map((event) => (
                      ...
                      >
                        <div className={`h-1 w-full ${event.color || "bg-blue-500"} rounded-sm`} />
                      </div>
                    ))}
                    {dayEvents.length > 3 && (
        ...
        <div className="flex flex-col space-y-2">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-2 mb-2">
            ...
            <div className="flex space-x-1">
              <Button className="h-8 text-xs px-2 flex-grow" onClick={handleFindEvents}>
                <Search className="h-3 w-3 mr-1" />
                Find
              </Button>
              <Button className="h-8 text-xs px-2" onClick={resetEvents} variant="outline">
                Reset
              </Button>
              <Button variant="secondary" className="h-8 text-xs px-2" onClick={() => setIsListView(!isListView)}>
      ...
      <CardContent className="p-2">
        {isListView ? (
          <div className="space-y-1 max-h-[calc(100vh-300px)] overflow-y-auto">
            {events.map((event) => (
              <div key={event.id} className="flex items-center justify-between p-1 border rounded-lg text-sm">
                ...
                {event.isSpecial && <Star className="h-3 w-3 text-yellow-500" />}
              </div>
            ))}
          </div>
        ) : (
              ...
              <thead>
                <tr>
                  {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
                    <th key={day} className="border p-1 text-left font-medium text-sm">
                      {day}
                    </th>
                  ))}
                </tr>
