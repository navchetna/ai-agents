...
const years = Array.from(
  new Set(resources.filter((r) => r.year).map((r) => r.year as number))
).sort((a, b) => b - a);
const grades = Array.from(new Set(resources.map((r) => r.grade))).sort(
  (a, b) => a - b
);
    ...
    <div className="container mx-auto py-6 space-y-6">
      ...
      <Tabs defaultValue="all">
        ...
        <TabsContent value="past_papers">
          <ResourceGrid
            resources={filteredResources.filter((r) => r.type === "past_paper")}
          />
        </TabsContent>
        <TabsContent value="notes">
          <ResourceGrid
            resources={filteredResources.filter((r) => r.type === "notes")}
          />
        </TabsContent>
        <TabsContent value="videos">
          <ResourceGrid
            resources={filteredResources.filter((r) => r.type === "video")}
          />
        </TabsContent>
      </Tabs>
    </div>
            ...
            <div className={`h-full ${getGradientClass(resource.subject)}`}>
              ...
              <CardContent>
                <div className="flex flex-wrap gap-2 items-center">
                  <Badge
                    variant="secondary"
                    className="bg-white/20 text-gray-900 dark:text-gray-100"
                  >
                    {resource.subject}
                  </Badge>
                  ...
                  {resource.year && (
                    <Badge
                      variant="outline"
                      className="bg-white/10 text-gray-900 dark:text-gray-100 border-gray-500"
                    >
                      {resource.year}
                    </Badge>
                  )}
...
function getGradientClass(subject: string): string {
  ...
  return `bg-gradient-to-br ${
    gradients[subject] || "from-gray-400/80 to-gray-300/80"
  }`;
}
