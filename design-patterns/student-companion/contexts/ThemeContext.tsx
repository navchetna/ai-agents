...
export const ThemeProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [scheduledTheme, setScheduledTheme] = useState<ScheduledTheme>(
    defaultScheduledTheme
  );
    ...
    const handleScheduledTheme = () => {
      if (scheduledTheme.enabled) {
        ...
        if (scheduledTheme.mode === "sunset") {
          ...
          const [lightHours, lightMinutes] = scheduledTheme.lightModeTime
            .split(":")
            .map(Number);
          const [darkHours, darkMinutes] = scheduledTheme.darkModeTime
            .split(":")
            .map(Number);
          ...
          if (lightModeTime < darkModeTime) {
            setMode(
              currentTime >= lightModeTime && currentTime < darkModeTime
                ? "light"
                : "dark"
            );
          } else {
  ...
  return (
    <ThemeContext.Provider
      value={{ mode, toggleColorMode, scheduledTheme, updateScheduledTheme }}
    >
