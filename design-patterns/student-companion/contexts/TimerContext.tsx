...
export const TimerProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    setTimeLeft(isStudySession ? timerSettings.studyDuration : timerSettings.breakDuration)
  ...
  useEffect(() => {
    if (focusEvents.length > 0 && sessionStartTime) {
      setCurrentFocusPercentage(calculateFocusPercentage(focusEvents, sessionStartTime, Date.now()))
    }
    ...
    if (isActive && timeLeft > 0) {
      interval = setInterval(() => {
        setTimeLeft((prevTime) => {
          ...
          const totalTime = isStudySession ? timerSettings.studyDuration : timerSettings.breakDuration
      ...
      if (isStudySession && sessionStartTime) {
        ...
        const sessionData: SessionData = {
          ...
          focusEvents,
          focusPercentage: calculateFocusPercentage(focusEvents, sessionStartTime, endTime),
        }
        setSessionHistory((prev) => [...prev, sessionData])
      }

      setIsStudySession((prev) => !prev)
      setTimeLeft(isStudySession ? timerSettings.breakDuration : timerSettings.studyDuration)
      setIsActive(false)
      setFocusEvents([])
  ...
  }, [isActive, timeLeft, isStudySession, focusEvents, sessionStartTime, calculateFocusPercentage, timerSettings])
  ...
  const addFocusEvent = (isFocused: boolean) => {
    setFocusEvents((prevFocusEvents) => [...prevFocusEvents, { timestamp: Date.now(), isFocused }])
  }
